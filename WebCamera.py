...
from handRecognition import BackgroundRemoval
	...
	def startcapture(self):					
		...
		while(1):
			...
			if (background_captured) :
				fg_frame = backgroundRemoval.background_remove(frame,background_model)
			if ( not (background_captured and capture_done)):
				frame,box_pos_x,box_pos_y = backgroundRemoval.set_background(frame,background_captured,capture_done)
			else:
